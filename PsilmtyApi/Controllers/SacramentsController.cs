...
public sealed class SacramentsController(IDatabaseRepository repository) : ControllerBase
{
    ...
    public async Task<IActionResult> Get() => Ok(await repository.QueryAsync<object>("""
        ...
        WHERE s.parish_id=@ParishId AND s.is_active=1 ORDER BY s.date DESC
        """, new { ParishId = User.GetParishId() }));

    [HttpGet("tipos")]
    public async Task<IActionResult> GetTypes() =>
        Ok(await repository.QueryAsync<object>("SELECT id Id,name Name FROM sacrament_types WHERE is_active=1 ORDER BY name"));
    ...
    public async Task<IActionResult> Create(Dictionary<string, object?> request)
    {
        var id = await repository.ExecuteScalarAsync<uint>("""
            INSERT INTO sacraments
                (parish_id,user_id,type_id,date,minister,godfather,godmother,record_number,observations,is_active,created_at,created_by)
            VALUES
    ...
    public async Task<IActionResult> Delete(uint id)
    {
        await repository.ExecuteAsync(
            "UPDATE sacraments SET is_active=0,updated_at=UTC_TIMESTAMP(),updated_by=@UserId WHERE id=@Id AND parish_id=@ParishId",
